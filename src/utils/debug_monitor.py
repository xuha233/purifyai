...
class DebugMonitor:
    ...
    def capture_error(self, exc: Exception, context: Dict[str, Any] = None) -> ErrorContext:
        ...
        # 防止递归调用导致无限循环
        global _recursion_depth, _MAX_RECURSION_DEPTH
        _recursion_depth += 1
        if _recursion_depth > _MAX_RECURSION_DEPTH:
            ...
            return ErrorContext(
                ...
                message=str(exc),
                traceback_str="(recursion limit exceeded)",
                timestamp=datetime.now(),
                context_info={'recursion_protected': True}
            )
        ...
        try:
            ...
            for callback in self.on_error_callbacks:
                try:
                    ...
                    # 回调失败时记录，但不再次触发 capture_error
                    try:
                        # 使用原生的 logging，避免再次进入 debug 监控
                        import logging as raw_logging
                        raw_root = raw_logging.getLogger()
                        raw_root.error(f"Error callback failed: {e}")
                    except:
                        pass

            # 记录到日志（使用原生 logging 避免递归）
            try:
                import logging as raw_logging
                raw_root = raw_logging.getLogger()
                raw_root.error(f"[ERROR:{exc_type}] {error_msg} | Context: {call_context}")
            except:
                pass  # 如果日志记录失败，静默失败
